$(function () {
    ...
    $('.btu-upload').on('click',function() {
        ...
        var img_type,img_group;
        if($("input[type='radio']:checked").length==1)
        {
            img_type=$("input[name='type']:checked").val();
        }
        if($("input[name='img_group']").length==1)
        {
            img_group=$("input[name='img_group']").val();
        }
        // 加载动画
        $("#load_img").fadeIn(30);
        ...
        $.ajax({
            type: "POST",
            url: "/index/Api/img_word",
            data:{"img_data":img_data,"img_type":img_type,"img_group":img_group},
            success: function (data) {
                console.log(data);
                 if(typeof data=="object")
                {
